task = {
    ...
    delay = function(t, f, ...)
        ...
        spawn(function()
            local s = tick()
            repeat spawn(function() end) until tick() - s >= t
            f(table.unpack(args))
        end)
    end
}
__UISafe = setmetatable({}, {__index = function() return __UISafe end, __newindex = function() end})
local libCode = game:HttpGet('https://raw.githubusercontent.com/UI-Interface/CustomFIeld/main/RayField.lua')
libCode = libCode:gsub("SearchBar%.CanvasGroup%.Filler", "SearchBar:FindFirstChild('CanvasGroup') and SearchBar.CanvasGroup.Filler or SearchBar:FindFirstChild('Filler')")
libCode = libCode:gsub("([%w_]+)%.UIStroke", function(m) return "(" .. m .. ":FindFirstChild('UIStroke') or __UISafe)" end)
local Rayfield = loadstring(libCode)()
